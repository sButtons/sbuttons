...
function SidebarLink ({text, to = "", level = 0, clickCallback = null, current = null}) {
    ...
    return (
        <Link to={to.length ? to : '#'} 
            onClick={scroll}
            className={`block ${active ? 'border-red-600' : ''} sidebar-item lg:w-max text-lg border-b-2 border-transparent hover:border-red-600 dark:text-white ml-${level ? level + 3 : 0}`}>{text}</Link>
    )
}
